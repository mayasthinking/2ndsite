...
import { mountBrushDial } from "./brush-dial.js?v=11";
...
function mountSheetEditor(sheet, item) {
  ...
  const placeBrushDial = () => {
    ...
    const radius = Math.round(Math.max(78, Math.min(painting.width * 0.26, painting.height * 0.28, staged ? 118 : 102)));
    const width = radius * 2;
    const height = radius;
    const apex = 10;
    const iconR = Math.round(radius * 0.7);
