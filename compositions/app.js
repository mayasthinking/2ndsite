...
import { mountBrushDial } from "./brush-dial.js?v=10";
...
function mountSheetEditor(sheet, item) {
  ...
  const placeBrushDial = () => {
    ...
    const width = Math.round(Math.max(176, Math.min(painting.width - 36, view.width - 32, staged ? 250 : 220)));
    const height = Math.round(Math.max(40, Math.min(staged ? 50 : 44, painting.height * 0.13)));
    const apex = 4;
    const radius = Math.round(Math.min(width * 0.68, 158));
    ...
    brushMenu.style.setProperty("--cy", `${height + radius - 20}px`);
    let left = painting.left + (painting.width - width) / 2;
    let top = painting.bottom - height + 2;
    left = Math.max(8, Math.min(left, view.width - width - 8));
    const minTop = Math.max(8, painting.top + 8);
    const maxTop = Math.min(view.height - height - 4, painting.bottom - height + 6);
