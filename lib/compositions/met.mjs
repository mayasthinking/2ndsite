...
export function metSearchUrl(search) {
  // The Met endpoint parses phrase searches correctly only when q is the final parameter.
  return `${MET_API}/search?hasImages=true&isPublicDomain=true&q=${encodeURIComponent(search.trim())}`;
}
...
const FIELD_FILTER_TERMS = {
  ...
  "romantic-wash": ["watercolor", "watercolour", "wash"],
};
...
export function rankMetItems(items, genreId = "", keyword = "") {
  const keywordTokens = tokenize(expandVisualQuery(keyword, 5));
  const scored = items.map((item, index) => {
    ...
    const catalogScore = identityScore(keyword, item);
    ...
    return {
      ...
      catalogMatch: catalogScore >= 0.5,
      score: genreMatchScore(genreId, searchable) * 3 + keywordScore + catalogScore * 4,
    };
  });
  const eligible = scored.filter((entry) => entry.primaryGenreMatch || entry.catalogMatch);
